...
interface MagicCardBaseProps {
  ...
  gradientFrom?: string;
  gradientTo?: string;
  /** Dark card: soft light interior hover. Light card: dark interior hover. Border ring uses gradientFrom/To (default: original Magic UI purple/pink). */
  surface?: "dark" | "light";
}
...
export function MagicCard(props: MagicCardProps) {
  const {
    ...
    gradientOpacity: gradientOpacityProp,
  ...
  const gradientFrom = useMemo(() => {
    if (gradientFromProp !== undefined) return gradientFromProp;
    return "#9E7AFF";
  }, [gradientFromProp]);

  const gradientTo = useMemo(() => {
    if (gradientToProp !== undefined) return gradientToProp;
    return "#FE8BBB";
  }, [gradientToProp]);

  const gradientColor = useMemo(() => {
    if (gradientColorProp !== undefined) return gradientColorProp;
    return darkSurface ? "rgba(255, 255, 255, 0.22)" : "#262626";
  }, [gradientColorProp, darkSurface]);

  const gradientOpacity =
    gradientOpacityProp ?? (darkSurface ? 1 : 0.8);

  const glowFrom = isOrbMode(props) ? (props.glowFrom ?? "#525252") : "#525252";
  const glowTo = isOrbMode(props) ? (props.glowTo ?? "#a3a3a3") : "#a3a3a3";
      ...
      {mode === "orb" && (
        <motion.div
          ...
          style={{
            ...
            mixBlendMode: darkSurface ? "screen" : "multiply",
